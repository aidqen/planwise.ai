...
export function PredefinedItemsDialog({ 
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-md w-[95vw] max-w-[500px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
                <DialogHeader>
                    ...
                    <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        {items.map((item, index) => (
                            ...
                            >
                                ...
                                <label 
                                    htmlFor={`item-${index}`} 
                                    className="flex flex-1 justify-between items-center text-sm font-medium text-gray-900 cursor-pointer dark:text-white"
                                >
                                    <span>
                                        {item.name}
                                        {item.importance && (
                                            <span className="ml-2 text-xs text-gray-500 capitalize dark:text-gray-400">
                ...
                <DialogFooter className="flex gap-2 justify-between items-center mt-4">
                    <DialogClose asChild>
                        <Button 
                            variant="destructive" 
                            className="md:mr-2 w-[12em] dark:text-white"
                        >
                            Cancel
                        </Button>
                    </DialogClose>
                    <Button 
                        ...
                        className="flex gap-2 items-center w-[12em] text-white bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700"
                    >
