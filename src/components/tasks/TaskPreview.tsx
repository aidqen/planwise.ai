...
export function TaskPreview({ task, onEdit, onDelete, actionsRight }: Props) {
  const title = task.title ?? task.name ?? task.summary ?? "Untitled"

  const range = ((): string | null => {
    if (task.startTime && task.endTime) return `${task.startTime} - ${task.endTime}`
    if (task.start && task.end) {
      if (task.start === '00:00' && task.end === '23:59') return null
  ...
  return (
    <div className="relative p-4 bg-white rounded-xl border border-gray-100 shadow-md transition-all duration-300 group hover:shadow-md dark:bg-gray-800/20 dark:hover:bg-gray-800/40 dark:border-gray-700/50 dark:hover:border-gray-600/50">
